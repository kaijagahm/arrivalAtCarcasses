...
list(
  ...
  tar_target(matrix_orders_nbda, map(roosts_bin_fixed_see, ~row.names(.x[[1]]))),
  tar_target(oas_nbda_numbers, map2(oa_see, matrix_orders_nbda, ~match(.x, .y))),
  ...
  tar_target(fl_mats_expanded, map(fl_allday_bin_fixed_see, ~map(.x, as.matrix))),
