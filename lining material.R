...
gmm.spring$gbi # group by individual matrix XXX won't have this for vultures but that's ok
gmm.spring$metadata
gmm.spring$B
...
TADA.finding.all <-
  tadaAICtable(
    nbdadata = list( # XXX KG they use multiple diffusions here.
      nbdaData_D1.all,
      nbdaData_D2.all,
      nbdaData_D3.all,
      nbdaData_D5.all),
...
igraph::plot.igraph( g.net,
                     ...
                     frame = FALSE,
                     layout=layout_with_graphopt(g.net),
                     asp = 1,
                     #     mark.groups = list.D,
                     #    mark.border =NA,
                     #  mark.col=col.adj
