...
all_bouts_annotated %>%
  ...
  group_by(carcID) %>%
  summarize(n = n()) %>%
  arrange(desc(n)) # let's pick another one with slightly fewer bouts: 4859212

mycarc <- aca %>%
  filter(carcID == "4859212")
...
glimpse(coflight)
t_g
t_g %>% activate(nodes) %>% pull(name) %>% length() # 59 vultures included in the co-flight network (presumably this includes one individual that was not present during the carcass time, only the day before, which accounts for only 58 individuals being detected in the foraging network)
...
# 53 (out of a network of 58-59 depending on the day)
...
mantel(coflight_day1, coflight_day2) # a significant mantel test would suggest that the matrices are significantly correlated. This is significant: 0.001, with a mantel statistic of 0.27 (decently correlated). Huh, interesting! That actually surprises me.

# I would expect the same between coflight_day2 and coflight_day3
mantel(coflight_day2, coflight_day3) # This time not significantly correlated-- significance 1, with a statistic -0.01007.

# Confirms that we're going to need dynamic foraging networks. sometimes it's correlated, sometimes not.
