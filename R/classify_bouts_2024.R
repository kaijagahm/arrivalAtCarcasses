...
# tags we're expecting
tags <- readxl::read_excel(here("data/ACC/2024_hf_period/raw/list_tags_high_res_Final.xlsx"))
tags_hf_2024 <- tags$local_identifier

## 1. Uploading raw ACC data
unobs_raw_acc <- list.files(path = "data/ACC/2024_hf_period/raw/",
                            pattern = "*.csv", full.names = T) %>%
  map_df(~read_csv(.), .progress = T) %>%
  mutate(UTC_datetime = as.POSIXct(UTC_datetime,
                                   format = c("%Y-%m-%d %H:%M:%S"),
                                   tz = "UTC")) # this takes a long time!!

save(unobs_raw_acc, file = here("data/ACC/2024_hf_period/created/unobs_raw_acc.Rda"))
load(here("data/ACC/2024_hf_period/created/unobs_raw_acc.Rda"))

# Examine the tags that are included in this
length(unique(unobs_raw_acc$device_id))
length(tags_hf_2024) # missing several...
...
#   ungroup()
# save(stat_feats, file = here("data/ACC/2024_hf_period/created/stat_feats.Rda"))
load(here("data/ACC/2024_hf_period/created/stat_feats.Rda"))
