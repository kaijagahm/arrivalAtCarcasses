...
nbda_data <- data_cumul_wt_3[[7]] # carcass 4422323, 
...
write_rds(gps_fornetwork, file = "data/created/gps_2.RDS")
write_rds(gps_fornetwork_day1, file = "data/created/gps_2_day1.RDS")
write_rds(first_sightings, file = "data/created/first_sightings_2.RDS")
write_rds(first_sightings_day1, file = "data/created/first_sightings_2_day1.RDS")
...
# For right-censored individuals, need fewer cutpoints. Need to end up with 32 networks for 32 individuals. So I need 33 cutpoints.
...
length(cutpoints_norightcens) # need 33 cutpoints so we can have 33 bins so we can define 32 events plus censored indivs.
...
write_rds(networks_long_dynamic, file = "data/created/networks_long_dynamic_2_fixed.RDS")
write_rds(networks_long_dynamic_norightcens, file = "data/created/networks_long_dynamic_2_fixed_norightcens.RDS")

networks_long_dynamic <- readRDS("data/created/networks_long_dynamic_2_fixed.RDS")
networks_long_dynamic_norightcens <- readRDS("data/created/networks_long_dynamic_2_fixed_norightcens.RDS")
...
prop_informed <- readRDS("data/created/prop_informed2.RDS")
...
write_rds(data_list, file="data/data_lists/datalist_2_fixed.RDS")
...
data_list_norightcens <- import_user_STb(event_data = event_data_nocensored, 
                                         ...
                                         ILVi = c("age", "mean_dist_to_carcass_norm", "prop_informed_norm"),
                                         ILVs = c("age", "prop_informed_norm")) 
write_rds(data_list_norightcens, file="data/data_lists/datalist_2_fixed_norightcens.RDS")

# Models
model_full_dynamic <- generate_STb_model(data_list, gq = T, est_acqTime = T)
write(model_full_dynamic, file="data/stan_models/model_2_fixed.stan")

model_full_dynamic_norightcens <- generate_STb_model(data_list_norightcens, gq = T, est_acqTime = T)
write(model_full_dynamic_norightcens, file="data/stan_models/model_2_fixed_norightcens.stan")
...
fit_dynamic <- fit_STb(data_list,
                       ...
                       iter = 500,
                       refresh=50)
STb_save(fit_dynamic, output_dir = "data/cmdstan_saves", name="dynamic_daylight_ilvs2_fixed")
fit_dynamic <- readRDS('data/cmdstan_saves/dynamic_daylight_ilvs2_fixed.rds') 
...
fit_dynamic_norightcens <- fit_STb(data_list_norightcens,
                                   ...
                                   iter = 500,
                                   refresh=50)
STb_save(fit_dynamic_norightcens, output_dir = "data/cmdstan_saves", name="dynamic_daylight_ilvs2_fixed_norightcens")
fit_dynamic_norightcens <- readRDS('data/cmdstan_saves/dynamic_daylight_ilvs2_fixed_norightcens.rds')
...
asocial_fit = fit_STb(data_list,
                      ...
                      iter = 500,
                      refresh=50)
STb_save(asocial_fit, output_dir = "data/cmdstan_saves", name="dynamic_daylight_ilvs_asoc2_fixed")
asocial_fit <- readRDS('data/cmdstan_saves/dynamic_daylight_ilvs_asoc2_fixed.rds') 
...
ggplot(comparison_df, aes(x = reorder(model, elpd_diff), y = elpd_diff)) +
  ...
  coord_flip() +
  labs(x = "Model", y = "ELPD Difference", title = "Carcass 2 ('fixed')") +
  theme_minimal()
