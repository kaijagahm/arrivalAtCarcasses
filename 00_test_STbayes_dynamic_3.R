...
nbda_data <- data_cumul_wt_3[[6]] # carcass 4420641, which is the 6th element of the 3rd list of 10, so element 26 of stn_carcs
...
# write_rds(networks_long_dynamic, file = "data/created/networks_long_dynamic_3.RDS")
networks_long_dynamic <- readRDS("data/created/networks_long_dynamic_3.RDS")
...
prop_informed3 <- readRDS("data/created/prop_informed3.RDS")
...
informed_list <- map(dates$date_il, ~{
  prop_informed3 %>% filter(roost_date == .x-lubridate::days(1))
})
...
write_rds(data_list, file="data/data_lists/dynamic_daylight_ilvs3.RDS")
...
write(model_full_dynamic, file="data/stan_models/dynamic_daylight_ilvs3.stan")
...
fit_dynamic <- fit_STb(data_list,
                       ...
                       iter = 500,
                       refresh=50)
STb_save(fit_dynamic, output_dir = "data/cmdstan_saves", name="dynamic_daylight_ilvs3")
fit_dynamic <- readRDS('data/cmdstan_saves/dynamic_daylight_ilvs3.rds') 
...
asocial_fit = fit_STb(data_list,
                      ...
                      iter = 500,
                      refresh=50)
STb_save(asocial_fit, output_dir = "data/cmdstan_saves", name="dynamic_daylight_ilvs_asoc3")
asocial_fit <- readRDS('data/cmdstan_saves/dynamic_daylight_ilvs_asoc3.rds') 
...
ggplot(comparison_df, aes(x = reorder(model, elpd_diff), y = elpd_diff)) +
  ...
  coord_flip() +
  labs(x = "Model", y = "ELPD Difference", title = "Carcass 3") +
  theme_minimal()
...
plots <- purrr::map(networks_long, ~{
  .x %>% filter(flight_sri > 0) %>% ggraph()+geom_node_point()+geom_edge_link() + geom_node_text(aes(label = name))+theme_void()
})
