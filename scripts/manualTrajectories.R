...
# Choose one to test
carc <- stn_carcs[[3]] # Hever, 4205250, 2022-11-16
gps <- stn_gps_30days[[3]] %>% 
  mutate(timestamp_il = lubridate::with_tz(timestamp, tz = 
                                             "Israel")) %>%
  filter(timestamp_il >= carc$datetime_il) %>%
  mutate(date_il = lubridate::date(timestamp_il),
         day = as.numeric(difftime(date_il, lubridate::date(carc$datetime_il), units = "days"))) %>%
  arrange(timestamp_il) %>%
  select(ground_speed, heading, height_above_msl, timestamp, tag_id, individual_id, individual_local_identifier, nick_name, sex, tag_local_identifier, year, date, dateOnly, dist_to_carcass, time_since_carcass, carcID, location_long, location_lat, timestamp_il, date_il, day)
...
carc_buffered <- st_buffer(carc, 4000)
